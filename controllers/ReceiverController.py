...
# Data received
dataReceived = open(str(os.environ['MISC']), mode='wb')
        ...
        with open(str(os.environ['RECEIVER_LOG_FILENAME']), encoding='utf-8', mode='w') as PKTLogger:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                while True:
                    ...
                    try:
                        decoded_data = AuxProcessing.BinaryToUTF8(TCPPkt.data)
                        dataReceived.write(decoded_data.encode('utf-8'))
                    except UnicodeDecodeError as e:
                        print("Error decoding UTF-8 data:", e)
                    ...
                    # receive the information that we requested for
                        ...
                        if GoBackN.ReceiveACK(TCPPkt):
                            ...
                            TCPPkt.sequence_number = AuxProcessing.IntegersToBinary(AuxProcessing.BinaryToIntegers(
                                TCPPkt.sequence_number) + int(os.environ['DEFAULT_WINDOW_SIZE']))
                        else:
